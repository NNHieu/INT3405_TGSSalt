...
def pred_all(model, test_dl, tta_flip):
    preds_test_upsampled = []
    model.eval()
    model.cuda()
    with torch.no_grad():
        for images in tqdm(test_dl):
            pred_masks = pred_batch(model, images, tta_flip)
...
def build_summission(preds_101):
    t1 = time.time()
    pred_dict = {idx: rle_encode(preds_101[i]) for i, idx in enumerate(tqdm(dm.ds_test.image_ids))}
    t2 = time.time()
...
def submit_model(model, data_root, add_depth, resize_pad, tta_flip, batch_size=16, num_workers=2):
    ...
    ds_test = TGSSaltDataset(data_root, test_df, image_set='test', transforms=TGSTransform(augment=False, add_depth=add_depth,resize_pad=resize_pad))
    ...
    sub = build_summission(preds_101)
