...
class PyramidPooling(nn.Module):
    ...
    def spatial_pyramid_pool(previous_conv, levels, mode):
        ...
        num_filter = previous_conv.size(1)
        ...
        for i in range(len(levels)):
            ...
            x = pool(padded_input) # [bs, filers, level[i], level[i]]
            if i == 0:
                spp = x.view(num_sample, num_filter, -1) # [bs, filers, level[i] * level[i]]
            else:
                spp = torch.cat((spp, x.view(num_sample, num_filter, -1)), 2) # [bs, filers, level[i] * level[i]]
...
class AttentionPooling(nn.Module):
    def __init__(self, input_size, identity=False, key_dim_reduction=2):
        super().__init__()
        self.att_layer = ScaleDotProductAttention(input_size, identity, key_dim_reduction)
        self.q_select = torch.FloatTensor([
                                            [0, 0, 0],
                                            [0, 1, 0],
                                            [0, 0, 0]
                                        ]).expand(1, input_size ,3, 3)
        # self.q_conv = nn.Conv2d(input_size, input_size, kernel_size=3)
        self.size = 3

    def forward(self, x):
        bs, c, w, h = x.size()
        # Q = conv2d(x_pad, self.q_select, stride=2)
        kw, kh = 3, 3
        dw, dh = 2, 2
        add_pad = (w - 1) % 2 == 1
        x_pad = F.pad(x, [1, 1 + add_pad, 1, 1 + add_pad])
        windows = x_pad.unfold(2, kh, dh).unfold(3, kw, dw).contiguous().view(1, 2, (w + 2) // 2, (h + 2) // 2, kw * kh)
        # # Q = self.q_conv(x_pad)
        # window = x.unfold()
        # for i in range(w):
        #     for j in range(h):
        #         pi, pj = i + 1, j + 1
        #         window = x[:, :, pi - 1:pi + 1, pj - 1:pj + 1].view(bs, c, -1)
        return windows
